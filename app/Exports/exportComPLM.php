...
class exportComPLM implements FromCollection,WithHeadings,WithMapping
{
    ...
    public function map($invoice): array
    {
           ...
           if(@$totalComFN >= 1000){
                @$totalComFNVat = round(((@$totalCom * 60) / 100) - ((((@$totalCom * 60) / 100)) * 3) /100);   //ไฟแนนซ์หลังหัก 3 %
           } else {
                @$totalComFNVat = round(@$totalComFN) ;   //ไฟแนนซ์หลังหัก 3 %
           }

           if(@$totalComAD >= 1000){
                @$totalComADVat = round(((@$totalCom * 40) / 100) - ((((@$totalCom * 40) / 100)) * 3) /100); // แอดมินหลังหัก 3 %
            } else {
                @$totalComADVat = round(@$totalComAD) ; // แอดมินหลังหัก 3 %
            }
